...
public final class KafkaUtils {
    ...
    public static ZkClient createZkClient(String zkConnection) {
    ...
    public static List<String> listTopics(ZkClient zkClient) {
        return zkClient.getChildren("/brokers/topics")
                .parallelStream()
                .collect(Collectors.toList());
    }

    public static List<Integer> listPartitions(ZkClient zkClient, String topicName) {
    ...
    public static String listBrokers(ZkClient zkClient) {
        List<String> brokers = new ArrayList<>();

        zkClient.getChildren("/brokers/ids").forEach(id -> {
            Broker broker = Broker.createBroker(
                    Integer.valueOf(id),
                    zkClient.readData("/brokers/ids/" + id, false)
            );
    ...
    public static Producer<String, SimplifiedLog> createProducer(ZkClient zkClient, KafkaProducerType type, boolean acks) {
        Validators.isNotNull(type, "type");

        Properties producerProperties = new Properties();
        producerProperties.putAll(COMMON_PROPS);
        producerProperties.put("metadata.broker.list", listBrokers(zkClient));
